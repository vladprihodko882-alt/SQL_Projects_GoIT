...
prev_month_data AS (
	SELECT 
		date_trunc('month', ad_date)::date 	AS add_month,
		campaign_name,
...
SELECT 
	...
	campaign_name,
	total_reach,
	COALESCE(LAG(total_reach) OVER (PARTITION BY campaign_name ORDER BY add_month), 0) 						AS prev_month_reach,
	ABS(total_reach - COALESCE(LAG(total_reach) OVER (PARTITION BY campaign_name ORDER BY add_month), 0)) 	AS abs_increase
